    ...
    */
    ...
    {
        if(step > ans_k) return;//history answers prune
        dp[s] = step;
        if(s == b)
        {
        ...
        {
            int pos = -1;
            pos = s.find(rule[i].from);
            while(pos != -1)
            {
                string changed = s;
                changed.erase(pos,rule[i].from.size());
                changed.insert(pos,rule[i].to);
                pos = s.find(rule[i].from,pos + 1);
                    if(dp.find(changed) == dp.end() || dp[changed] > step + 1) dfs(changed , step + 1);
